...
#
# Usage:
# sbatch experiments/updated_eprop_model_sweep/submit_sweep_slurm.sh
#
# Behavior:
...
submit_analysis_job() {
  ...
  submit_out="$(sbatch \
    ...
    --wrap "bash -lc 'cd \"$REPO_ROOT\" && source \"$REPO_ROOT/env_load_hambach.sh\" && python -m motor_controller_model.analyze_sweep --sweep-root \"$sweep_root\" --top-k 10 --promote'"
  )"
