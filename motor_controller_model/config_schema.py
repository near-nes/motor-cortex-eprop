...
class SimulationConfig(BaseModel):
    ...
    total_num_virtual_procs: int = Field(default=4, description="Number of virtual processes for NEST")
...
class TaskConfig(BaseModel):
    ...
    n_samples_per_trajectory_to_use: int = Field(
        default=1, description="How many samples to take from the start of each trajectory (legacy)"
    )
    gradient_batch_size: int = Field(default=1, description="Batch size for gradient computation")
    ...
    silent_period: float = Field(default=0.0, description="Silent period before receiving the trajectory (ms)")
    input_shift_ms: float = Field(default=100.0, description="Temporal delay to shift M1 target spikes forward (ms)")
    learning_start: float = Field(
        default=650.0, description="Start time (ms) inside one sequence where learning becomes active"
    )
    learning_end: float = Field(
        default=1500.0, description="End time (ms) inside one sequence where learning stops"
    )
...
class RecurrentNeuronConfig(BaseModel):
    ...
    regular_spike_arrival: bool = Field(default=False, description="Use regular spike arrival")
...
class OutputNeuronConfig(BaseModel):
    ...
    E_L: float = Field(default=0.0, description="Resting membrane potential (mV)")
    I_e: float = Field(default=0.0, description="External current (pA)")
    loss: str = Field(default="mean_squared_error", description="Loss function for output neurons")
    regular_spike_arrival: bool = Field(default=False, description="Use regular spike arrival")
    tau_m: float = Field(default=20.0, description="Membrane time constant (ms)")
    V_m: float = Field(default=0.0, description="Initial membrane potential (mV)")
...
class OptimizerConfig(BaseModel):
    ...
    type: Literal["gradient_descent"] = Field(default="gradient_descent", description="Optimizer type")
...
class InhSynapseConfig(BaseModel):
    ...
    weight: float = Field(default=-400.0, description="Initial inhibitory synaptic weight (pA)")
...
class SynapsesConfig(BaseModel):
    ...
    conn_bernoulli_p: float = Field(default=0.1, description="Connection probability for recurrent connections")
    average_gradient: bool = Field(default=False, description="Average gradient across batch")
    static_delay: float = Field(default=1.0, description="Delay for static synapses (ms)")
    feedback_delay: float = Field(default=1.0, description="Delay for feedback synapses (ms)")
    rate_target_delay: float = Field(default=1.0, description="Delay for rate target synapses (ms)")
    receptor_type: int = Field(default=2, description="Receptor type for rate target synapses")
...
class MultimeterOutConfig(BaseModel):
    ...
    interval: float = Field(default=1.0, description="Recording interval (ms)")
    record_from: List[str] = Field(
        default=["V_m", "readout_signal", "readout_signal_unnorm", "target_signal", "error_signal"],
