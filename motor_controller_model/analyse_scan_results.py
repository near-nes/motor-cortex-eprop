...
"""
Analyse scan results from motor-controller-model parameter sweeps.

This module reads the scan_summary.csv file generated by eprop_reaching_task.py parameter scans,
...
The heatmaps are saved as PNG files in the sim_results directory at the repository root.

- Axes are automatically detected from columns with more than one unique value (excluding known metadata columns).
- The heatmap always uses a log10 scale for better visibility.
- The module is headless and does not display plots, only saves them.
...
csv_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "sim_results", "scan_summary.csv")
